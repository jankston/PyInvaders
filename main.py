...
def isBombCollision(enemyX, enemyY, bombX, bombY):
...
while running:
    ...
    for i in range(num_of_enemies):
        ...
        # Collision
        bullet_collision = isBulletCollision(enemyX[i], enemyY[i], bulletX, bulletY)
        bomb_collision = isBombCollision(enemyX[i], enemyY[i], bombX, bombY)
        ...
        if bomb_collision:
            bombSound = mixer.Sound("bomb.wav")
            bombSound.play()
            bombY = 480
            score_value += 1
            bomb_state = "ready"
